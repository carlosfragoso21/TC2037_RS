...
UNASSIGN|ENDSELECT|ENDLOOP|ENDFORM|CONTINUE|CONVERT|DESCRIBE|COLLECT|COMMIT|COMPUTE|SELECTION|FUNCTION|ROLLBACK|REFRESH|REJECT|REPORT|RETURN|METHOD|MODIFY|MODULE|MULTIPLY|EXPORT|EXTRACT|APPEND|OBJECT|CATCH|CHECK|CLASS|CLEAR|CLOSE|OPTION|ENDTRY|UNION|UPDATE|VALUE|VALUES|VARYING|WHEN|WHILE|WITH|WRITE|WHERE|REPORT|
...
"
  ...
  defp conform_html(lv_html_path, lv_name_content) do
    [filen, _] = Path.basename(lv_html_path)
    |> String.split(".")
    file_cont_head_html =
"<!DOCTYPE html>
<html>
  ...
  <link rel=\"stylesheet\" type=\"text/css\" href=\" " <> filen <> ".CSS" <> "\" />
  </style>
  </head>
  <body> \n <PRE>
"
...
", 'A')
  ...
  def main_syntax(p_path) do
    ...
    |> String.split(".")
    lv_css_path = Path.dirname(p_path) <> "\/" <> filen <> ".CSS"
